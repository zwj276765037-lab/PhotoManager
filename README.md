...
当前阶段：产品已调整为“GIF 动态图片或视频均可输入，生成动态杂志海报后可选择 GIF、MP4 或两者输出”。`goal/README.md` 为 v0.5 待审核稿；`workspace/demo/` 已提供双输入双输出、三种结构和清晰度对比样片，正式小程序仍需在方案审核后实现。
...
用户主动选择一段短视频或一个 GIF，小程序负责素材预览、高光确认、输出格式和轻量编辑；云端负责运动分析、主体分层、杂志版式、循环编排，以及 MP4/GIF 渲染。第一版采用以下数据流：
