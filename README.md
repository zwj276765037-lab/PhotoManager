...
当前阶段：先用“电脑本地网站＋手机同一 Wi-Fi 访问”验证产品。iPhone 可上传 GIF/视频；Live Photo 先在照片 App 中“存储为视频”以保留真实动作，再上传处理。结果可选择 GIF、MP4 或两者，并附 JPG 封面。`goal/README.md` 为 v0.7 待审核稿；`workspace/web/` 已提供真实上传、异步渲染、移动端操作、预览和保存。iOS App 原生成对读取保留为后续预案。
...
用户在电脑启动本地网站，手机连接同一可信 Wi-Fi 后打开局域网地址。网站负责上传、任务状态、素材预览、输出格式和结果保存；电脑负责运动分析、杂志版式、循环编排和 MP4/GIF/JPG 渲染。第一版采用以下数据流：

```text
Live Photo 先在 iPhone 照片 App 中“存储为视频”
        ↓
手机网站选择转换后的视频、普通视频或 GIF
        ↓
局域网上传到电脑任务目录并完成输入检查
        ↓
...
1. 本地任务文件：普通视频/GIF、代理视频、主体蒙版和预览，保存在 Git 忽略的运行目录。
