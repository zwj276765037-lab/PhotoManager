...
替换 `--source` 就可以测试自己的 GIF。当前命令行样片中的“动态图片”明确指 GIF；产品 v0.6 已将原生 iOS Live Photo 读取设为 P0，但需要先通过 iOS PhotoKit 原型取得静态照片与 pairedVideo，尚未直接接入这条命令行验证链路。APNG 和动态 WebP 也暂未纳入。
...
- 本地网页的自动循环不代表朋友圈一定自动重播。iOS 首发必须把样片保存到真实 iPhone 并发布到测试朋友圈回看；后续小程序或 Android 版本需另做对应真机验证。
