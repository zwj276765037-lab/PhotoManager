...
替换 `--source` 就可以测试自己的 GIF。当前命令行样片中的“动态图片”明确指 GIF；产品 v0.7 先做手机可访问的本地网站，Live Photo 使用 iPhone“存储为视频”后上传，原生 PhotoKit 成对读取保留为后续 App 预案。APNG 和动态 WebP 暂未纳入。
...
- 本地网页的自动循环不代表朋友圈一定自动重播。网站首发必须在真实 iPhone Safari 下载样片并发布到测试朋友圈回看；后续 App、小程序或 Android 版本需另做对应真机验证。
